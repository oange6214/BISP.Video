...
public class ObservableVideoCaptureDevice : IObservableVideoSource, IDisposable
{
    #region Fields
    ...
    private readonly Subject<VideoSourceErrorEventArgs> _errorSubject = new();
    private readonly Subject<IntPtr> _frameSubject = new();
    private readonly Subject<ReasonToFinishPlaying> _playingFinishedSubject = new();

    public IObservable<VideoSourceErrorEventArgs> ErrorStream => _errorSubject.AsObservable();
    public IObservable<IntPtr> FrameStream => _frameSubject.AsObservable();
    public IObservable<ReasonToFinishPlaying> PlayingFinishedStream => _playingFinishedSubject.AsObservable();
    ...
    /// </summary>
    ///
    public ObservableVideoCaptureDevice()
    { }
    ...
    ///
    public ObservableVideoCaptureDevice(string deviceMoniker)
    {
    ...
    ///
    public IObservable<Unit> WaitForStop()
    {
    ...
    public void Dispose()
    {
        ...
        _playingFinishedSubject.Dispose();
    }
    ...
    ///
    private void OnNewFrame(IntPtr image)
    {
        _framesReceived++;
        //_bytesReceived += image.PixelWidth * image.PixelHeight * (image.Format.BitsPerPixel >> 3);

        if (!_stopEvent.WaitOne(0, false) && _frameSubject != null)
        {
    ...
    private void WorkerThread(bool runGraph)
    {
        ...
        catch (Exception exception)
        {
            ...
            _errorSubject?.OnNext(new VideoSourceErrorEventArgs(exception.Message));
        }
        finally
        {
            // release all objects
        ...
        _playingFinishedSubject?.OnNext(reasonToStop);
    ...
    {
        private ObservableVideoCaptureDevice parent;
        private bool snapshotMode;
        private int width, height;

        // Constructor
        public Grabber(ObservableVideoCaptureDevice parent, bool snapshotMode)
        {
            this.parent = parent;
